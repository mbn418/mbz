    ...
    public class AuditModel : ModelBase
    {
        ...
        public object SelectLists
        {
            get
            {
                return new
                {
                    LogTables = AuxFunctions.ToSelectList<LogTables>()
                };
            }
        }
        ...
        public void Where(PersistentSupport sp, int offset, int numberListItems, CriteriaSet filter, List<ColumnSort> sortColumns, bool isExport, string year)
        {
			// Retrieve view details
            AreaInfo viewInfo = null;
            string viewName;

            if (LogTable == AuditModel.LogTables.logTRAall)
            {
            ...
            else
            {
                ...
                viewName = AuditModel.LogTables.logTRAall.ToString() + "_" + LogTable.ToString() + "_VIEW";
            }
            ...
            else if (LogTable == AuditModel.LogTables.logTRAall)
            {
                query
            ...
            if (this.LogDatabaseSelected)
                systemSp = PersistentSupport.getPersistentSupport(year, "");
            ...
            for (var i = 0; i < values.NumRows; i++)
            {
                ...
                else
                {
                    ...
                    for (var j = 0; j < camposLog.Count; j++)
                    {
                        ...
                        text = GetHumanValue(systemSp, viewInfo, field, text);
		...
		public void FillSelectedRowValues(PersistentSupport sp)
        {
            ...
            for (var j = 0; j < camposLog.Count; j++)
            {
                ...
                text = GetHumanValue(sp, area, field, text);
        ...
        private static string GetHumanValue(PersistentSupport sp, AreaInfo area, CSGenio.framework.Field field, string text)
        {
                     ...
                     !string.IsNullOrEmpty(field.ArrayName) && !string.IsNullOrEmpty(text))
            {
                ...
                try
                {
                    ...
                    text = array.GetDescription(text, /*dbSearch.Language*/"");
                }

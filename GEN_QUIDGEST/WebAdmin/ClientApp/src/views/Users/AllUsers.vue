<template>
	...
	<qtable 
		...
		<template #TRA="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'TRA' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					TRA : { active: true, Cod: "TRA", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.TRA.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.TRA.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.TRA.Description),
					name: "TRA",
					slot_name: "TRA",
					sort: false,
					visibility: vm.Modules.TRA.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {

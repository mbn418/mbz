...
namespace GenioMVC.Controllers
{
	public partial class Photo_albumController : ControllerBase
	{
		...
		public Photo_albumController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL TRA CONTROLLER_NAVIGATION PHOTO_ALBUM]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAphoto_album>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL TRA MANUAL_CONTROLLER PHOTO_ALBUM]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Photo_album row = new Models.Photo_album(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("photo_album");
			...
			try
			{
				...
				{
					case "PHOTO__PROPERTY__TITLE":	// Field (DB)
						{
							var model = new Photo_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Photo__property__title(qs);
							result = model.TablePropertyTitle;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "PHOTO__PROPERTY__TITLE":	// Field (DB)
						values = new Photo_ViewModel(UserContext.Current).GetDependant_PhotoTablePropertyTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Photo" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Photo([FromBody]Photo_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "photo_album",
				(primaryKey) => Models.Photo_album.Find(primaryKey, UserContext.Current, "FPHOTO"),
				(model) => formData.MapToModel(model as Models.Photo_album)
			);
		}
		...
		{
			return base.GetDocumsTickets("PHOTO_ALBUM", requestModel.FieldName, requestModel.KeyValue);
		}

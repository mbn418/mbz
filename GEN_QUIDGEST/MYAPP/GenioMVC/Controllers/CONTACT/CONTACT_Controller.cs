...
namespace GenioMVC.Controllers
{
	public partial class ContactController : ControllerBase
	{
		...
		public ContactController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL TRA CONTROLLER_NAVIGATION CONTACT]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAcontact>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL TRA MANUAL_CONTROLLER CONTACT]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Contact row = new Models.Contact(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("contact");
			...
			try
			{
				...
				{
					case "CONTACT__PROPERTY__TITLE":	// Field (DB)
						{
							var model = new Contact_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Contact__property__title(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "CONTACT__PROPERTY__TITLE":	// Field (DB)
						values = new Contact_ViewModel(UserContext.Current).GetDependant_ContactTablePropertyTitle(Selected);
						break;
					default: break;
				}
		...
		{
			return GenericRecalculateFormulas(formData, "contact",
				(primaryKey) => Models.Contact.Find(primaryKey, UserContext.Current, "FCONTACT"),
				(model) => formData.MapToModel(model as Models.Contact)
			);
		}
		...
		{
			return base.GetDocumsTickets("CONTACT", requestModel.FieldName, requestModel.KeyValue);
		}

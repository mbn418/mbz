...
// USE /[MANUAL TRA CONTROLLER_NAVIGATION CITY]/
...
// USE /[MANUAL TRA MANUAL_CONTROLLER CITY]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.City row = new Models.City(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("city");
			...
			try
			{
				...
				{
					case "CITY__COUNTRY__COUNTRY":	// Field (DB)
						{
							var model = new City_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_City__country__country(qs);
							result = model.TableCountryCountry;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "CITY__COUNTRY__COUNTRY":	// Field (DB)
						values = new City_ViewModel(UserContext.Current).GetDependant_CityTableCountryCountry(Selected);
						break;
					default: break;
				}
		...
		{
			return base.GetDocumsTickets("CITY", requestModel.FieldName, requestModel.KeyValue);
		}

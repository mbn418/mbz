...
{
	public class Country_ViewModel : FormViewModel<Models.Country>, IPreparableForSerialization
	{
		[JsonIgnore]
		...
		public Country_ViewModel(UserContext userContext, Models.Country row, bool nestedForm = false) : base(userContext, "FCOUNTRY", row, nestedForm) { }
		...
		public Country_ViewModel(UserContext userContext, string id, bool nestedForm = false, string[]? fieldsToLoad = null) : this(userContext, nestedForm)
		{
			...
			Model = Models.Country.Find(id, userContext, "FCOUNTRY", fieldsToQuery: fieldsToLoad);
			if (Model == null)
		...
		public static StatusMessage InsertConditions(UserContext userContext)
		{
			...
			Models.Country model = new Models.Country(userContext) { Identifier = "FCOUNTRY" };
		...
		public override void MapFromModel(Models.Country m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map Model (Country) to ViewModel (Country) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error("Map Model (Country) to ViewModel (Country) - Error during mapping");
				throw;
			}
		}
		...
		public override void MapToModel(Models.Country m)
		{
			if (m == null)
			{
				CSGenio.framework.Log.Error("Map ViewModel (Country) to Model (Country) - Model is a null reference");
				throw new ModelNotFoundException("Model not found");
			}
			...
			{
				CSGenio.framework.Log.Error($"Map ViewModel (Country) to Model (Country) - Error during mapping. All user values: {HasDisabledUserValuesSecurity}");
				throw;
			}
		}
		...
		{
			try { Model = Models.Country.Find(id ?? Navigation.GetStrValue("country"), m_userContext, "FCOUNTRY"); }
			finally { Model ??= new Models.Country(m_userContext) { Identifier = "FCOUNTRY" }; }
		...
		public override void Load(NameValueCollection qs, bool editable, bool ajaxRequest = false, bool lazyLoad = false)
		{
			...
			try
			{
				Model = Models.Country.Find(Navigation.GetStrValue("country"), m_userContext, "FCOUNTRY");
			}
			finally
			{
				if (Model == null)
		...
		public override void LoadPartial(NameValueCollection qs, bool lazyLoad = false)
		{
			...
			if (Model == null)
			{
				...
				if (Model == null)
				{
					Model = new Models.Country(m_userContext) { Identifier = "FCOUNTRY" };
...
// USE /[MANUAL TRA VIEWMODEL_LOADPARTIAL COUNTRY]/
		}

// USE /[MANUAL TRA VIEWMODEL_NEW COUNTRY]/
...
// USE /[MANUAL TRA VIEWMODEL_SAVE COUNTRY]/
...
// USE /[MANUAL TRA VIEWMODEL_APPLY COUNTRY]/

// USE /[MANUAL TRA VIEWMODEL_DUPLICATE COUNTRY]/

// USE /[MANUAL TRA VIEWMODEL_DESTROY COUNTRY]/
		public override void Destroy(string id)
		{
			Model = Models.Country.Find(id, m_userContext, "FCOUNTRY");
			if (Model == null)
				throw new ModelNotFoundException("Model not found");
...
// USE /[MANUAL TRA VIEWMODEL_CUSTOM COUNTRY]/

	...
	<teleport
		...
		<q-container
			...
			<template v-if="formControl.initialized && showFormBody">
				...
				<q-row v-if="controls.F_BROKER__BROKER__NAME.isVisible || controls.F_BROKER__BROKER__BRITHDATE.isVisible">
					<q-col
					...
					<q-col
						v-if="controls.F_BROKER__BROKER__BRITHDATE.isVisible"
						cols="auto">
						<base-input-structure
							v-if="controls.F_BROKER__BROKER__BRITHDATE.isVisible"
							class="i-text"
							v-bind="controls.F_BROKER__BROKER__BRITHDATE"
							v-on="controls.F_BROKER__BROKER__BRITHDATE.handlers"
							:loading="controls.F_BROKER__BROKER__BRITHDATE.props.loading"
							:reporting-mode-on="reportingModeCAV"
							:suggestion-mode-on="suggestionModeOn">
							<q-date-time-picker
								v-if="controls.F_BROKER__BROKER__BRITHDATE.isVisible"
								v-bind="controls.F_BROKER__BROKER__BRITHDATE.props"
								:model-value="model.ValBrithdate.value"
								@reset-icon-click="model.ValBrithdate.fnUpdateValue(model.ValBrithdate.originalValue ?? new Date())"
								@update:model-value="model.ValBrithdate.fnUpdateValue($event ?? '')" />
						</base-input-structure>
					</q-col>
				</q-row>
		...
		data()
		{
			...
			return {
				...
				controls: {
					...
					F_BROKER__BROKER__BRITHDATE: new fieldControlClass.DateControl({
						modelField: 'ValBrithdate',
						valueChangeEvent: 'fieldChange:broker.brithdate',
						id: 'F_BROKER__BROKER__BRITHDATE',
						name: 'BRITHDATE',
						size: 'small',
				...
				dataApi: {
					Broker: {
						get ValBrithdate() { return vm.model.ValBrithdate.value },
						set ValBrithdate(value) { vm.model.ValBrithdate.updateValue(value) },

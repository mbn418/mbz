<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-f-property-city-city-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['F_PROPERTY__CITY__CITY_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FPropertyCityCitySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('F_PROPERTY__CITY__CITY_SeeMore', false),

				interfaceMetadata: {
					id: 'F_PROPERTY__CITY__CITY_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'F_PROPERTY__CITY__CITY_SeeMore',
					name: 'F_PROPERTY__CITY__CITY_SeeMore',
					controller: 'PROPERTY',
					action: 'F_PROPERTY_CityValCity'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-f-property-city-city',
				dismissAction: this.close,
				returnElement: 'F_PROPERTY__CITY__CITY_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.CITY35974),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-f-property-city-city')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'F_property_F_property_CityValCity',

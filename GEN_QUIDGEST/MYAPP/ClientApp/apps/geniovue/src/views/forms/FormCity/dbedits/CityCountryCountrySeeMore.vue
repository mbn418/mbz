<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-city-country-country-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['CITY__COUNTRY__COUNTRY_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'CityCountryCountrySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('CITY__COUNTRY__COUNTRY_SeeMore', false),

				interfaceMetadata: {
					id: 'CITY__COUNTRY__COUNTRY_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'CITY__COUNTRY__COUNTRY_SeeMore',
					name: 'CITY__COUNTRY__COUNTRY_SeeMore',
					controller: 'CITY',
					action: 'CITY_CountryValCountry'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-city-country-country',
				dismissAction: this.close,
				returnElement: 'CITY__COUNTRY__COUNTRY_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.COUNTRY64133),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-city-country-country')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'City_City_CountryValCountry',
